...
class AIService {
   ...
   */
  async mapCSVRecords(records, modelName = 'gemini-2.5-flash') {
    if (!ai) {
      throw new AppError(
    ...
    try {
      const prompt = buildCrmMappingPrompt(records);
      ...
      console.log(`Sending ${records.length} records to Gemini using model: ${modelName}`);
      ...
      try {
        ...
        throw new AppError('Gemini API returned invalid JSON format: ' + jsonError.message, 500);
      }
      ...
      return mappedResult
        ...
        .filter(row => {
          ...
          const hasMobile = row.mobile_without_country_code && row.mobile_without_country_code.trim().length > 0;
